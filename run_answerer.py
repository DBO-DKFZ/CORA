...
class AnswerGenerator:
    """
    Generates answers for multiple-choice medical questions.
    ...
    @staticmethod
    def _build_result_row(record: dict, llm_response: str, llm_response_raw: str, used_sources: str) -> dict:
        retrieval_result = record.get("retrieval_result")
        row = {
        ...
        if retrieval_result:
            ...
            row.update({
                ...
                "retrieval_iterations": sum(1 for s in history if s.get("query")),
    ...
    def _is_sufficient(self, retrieval_result: dict) -> bool:
        if self.min_confidence is not None:
            ...
            return last.get("sufficient", False) and last.get("confidence", 0) >= self.min_confidence
    ...
    def _load_rag_records(self) -> list:
        ...
        for filename in sorted(os.listdir(self.retrieved_docs_dir)):
            if not filename.endswith(".json"):
                continue
            with open(os.path.join(self.retrieved_docs_dir, filename), "r", encoding="utf-8") as f:
                record = json.load(f)
    ...
    def _load_checkpoint(self, records: list) -> tuple[list, list]:
        """Return (already_done_rows, todo_records). Skips question_ids already in output CSV."""
        if os.path.exists(self.output_csv):
            ...
            if self.retry_nan_responses:
                if self.open_ended:
                    ...
                    valid = existing_df["llm_response"].astype(str).str.match(r"^[A-Ea-e]$")
            ...
            print(f"Resuming: {len(done_ids)} already answered, {len(todo)} remaining.")
            return done_df.to_dict("records"), todo
        return [], records

    def _save_checkpoint(self, rows: list) -> None:
        pd.DataFrame(rows).to_csv(self.output_csv, index=False)
    ...
    def process(self):
        ...
        if self.rag_mode:
            ...
            records = [r for r in records if r["answer_source"] in ("agentic_rag", "agentic_rag_insufficient", "naive_rag")]
            if n_before - len(records):
                self._vprint(f"Skipping {n_before - len(records)} records with non-agentic answer_source.")

        existing_rows, todo = self._load_checkpoint(records)
        all_rows = list(existing_rows)

        for record in tqdm.tqdm(todo, desc="Generating answers"):

            if self.verbose:
                print(f"\n======== [{record['question_id']}] ========")
                print(f"  source:   {record['answer_source']}")
                ...
                if self.rag_mode:
                    ...
                    print(f"  docs:     {len(docs)}  iterations: {iterations}")

            final_prompt = self._build_final_prompt(record)
            llm_response_raw = None
            for attempt in range(1, self.max_retries + 2):
                try:
                    llm_response_raw = self._call_llm(final_prompt)
                    break
            ...
            self._vprint(f"  correct:  {record['correct_choice']}")

            all_rows.append(self._build_result_row(record, llm_response, llm_response_raw, used_sources))

            if len(all_rows) % 20 == 0:
                self._save_checkpoint(all_rows)
                print(f"  [checkpoint] saved {len(all_rows)} rows to {self.output_csv}")
        ...
        if self.verbose:
            if not df.empty:
                if "retrieval_iterations" in df.columns and df["retrieval_iterations"].gt(0).any():
                    print(f"Average retrieval iterations: {df['retrieval_iterations'].mean():.2f}")
                    print(f"Average documents retrieved:  {df['final_doc_count'].mean():.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--top_k", type=int, default=None, help="Truncate reranked docs to top-k at answerer time")
