...
class AgenticRetriever:
    ...
    def plan_retrieval_strategy(self, question: str) -> dict:
        """Ask the LLM which collections to search, whether to decompose, and what concepts to focus on."""
        prompt = f"""
        ...
        Determine:
        1. Should we search the primary medical collection, books collection, or both?
        2. Does this question require multiple sub-queries (e.g., definition + mechanism + treatment)?
        3. What are the key medical concepts/terms to focus on?

        Return a JSON object with:
        {{
            "collections_to_search": ["primary", "books"],
    ...
    def critique_retrieval(self, question: str, documents: List[str], iteration: int) -> dict:
        ...
        {{
            ...
            "confidence": 0.0-1.0,
            "gaps": ["gap1", "gap2"],
            "recommendation": "answer" | "retrieve_more" | "try_different_collection",
            "reasoning": "1 line explanation"
        }}
        """
    ...
    def retrieve_from_collection(self, query: str, collection) -> List[str]:
        """Embed query and return top-k matching documents from a ChromaDB collection."""
    ...
    def retrieve_for_queries(self, queries: List[str], collections_to_search: List[str]) -> List[str]:
        """Run retrieval across all queries and specified collections, returning deduplicated docs."""
        docs = []
        for query in queries:
            if "primary" in collections_to_search:
                docs.extend(self.retrieve_from_collection(query, self.collection))
            if "books" in collections_to_search:
                docs.extend(self.retrieve_from_collection(query, self.books_collection))
    ...
    def agentic_retrieval(self, question: str, question_id: str = "") -> dict:
        ...
        Full agentic retrieval pipeline:
          1. Plan the retrieval strategy (collections, decomposition).
        ...
        if self.verbose:
            print(f"{prefix}[PLAN] collections={strategy['collections_to_search']}  "
                  f"decompose={strategy['requires_decomposition']}")
        ...
        plan_metadata = {
            "collections": strategy.get("collections_to_search", []),
        ...
        def run_retrieval_iteration(queries: List[str], iteration: int) -> bool:
            ...
            new_docs = self.retrieve_for_queries(queries, strategy["collections_to_search"])
            ...
            if self.enable_self_critique and new_docs:
                ...
                if critique["recommendation"] == "try_different_collection":
                    if "books" not in strategy["collections_to_search"]:
                        strategy["collections_to_search"].append("books")
                        if self.verbose:
                            print(f"{prefix}[EXPAND] adding books collection to search strategy")
        ...
        if not sufficient:
            for i in range(1, self.max_iterations):
                ...
                reformulated = [self.reformulate_query(q, i) for q in queries]
                if self.verbose:
    ...
    def process(self, max_workers: int = 16):
