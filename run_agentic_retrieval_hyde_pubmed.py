"""
Gap-based agentic retrieval over the case-report collection ONLY.

Standalone variant of run_agentic_retrieval_gaps.py for the PubMed vignettes,
where the case-report corpus holds ~all the coverage (guidelines/books add only
~1.5% unique). It queries a single ChromaDB collection of PMC case reports
(built by ingest_case_reports.py) — there is no collection routing.

Kept from the gaps retriever: query decomposition, gap-aware iterative
reformulation, and self-critique. Removed: the plan step's collection choice,
primary/books collections, and the "try a different collection" fallback.

Leakage note: the case_reports collection must have been built with
ingest_case_reports.py, which excludes any report whose PMID/PMCID is a
vignette source — otherwise RAG retrieves the source article verbatim.

Usage:
    python run_agentic_retrieval_case_reports.py --config configs/agentic_retrieval_case_reports_pubmed.yaml
"""
...
class AgenticRetriever:
    ...
    def _call_agent(self, system_prompt: str, user_prompt: str) -> str:
        while True:
            try:
                ...
                if self.verbose:
                    print(f"[RETRY] error from LLM ({e}) — retrying in 5s...")
                time.sleep(5)
    ...
    def plan_retrieval_strategy(self, question: str) -> dict:
        """Identify the key medical concepts to focus on (recorded as `conditions` metadata)."""
        prompt = f"""
        Analyze this medical question and identify the key medical concepts to focus retrieval on.
    ...
    def hyde_queries(self, question: str, gaps: List[str] = None) -> List[str]:
        """
        Multi-hypothesis HyDE: propose the 3 most likely answers and write each as a
        case-report-style statement, so the query matches published case reports
        (which are written around the diagnosis/test/treatment, not the presentation).

        Facet-aware: if the question asks for the next diagnostic step or treatment,
        the hypotheses are oriented toward the confirmatory test / management of the
        likely entity — the second hop that raw retrieval misses. Gap-aware on
        reformulation: prior-iteration gaps steer the next hypotheses.
        """
        gaps_section = ""
        if gaps:
            gaps_list = "\n".join(f"  - {g}" for g in gaps)
            gaps_section = (f"\nPrior retrieval left these gaps — target them with more specific "
                           f"or alternative hypotheses:\n{gaps_list}\n")
        prompt = f"""
        You are retrieving published PubMed case reports to answer a clinical question.

        Propose the 3 most likely answers to what the question asks:
        - If it asks for the diagnosis: the 3 most likely specific diagnoses.
        - If it asks for the next diagnostic step: the confirmatory test/work-up for each of the
          top likely diagnoses.
        - If it asks for treatment: the management/therapy for each of the top likely diagnoses.

        Write each as ONE sentence in the style of a case-report title or conclusion that names the
        specific entity (disease, test, or therapy) and its hallmark features, so it retrieves
        similar published case reports.
        {gaps_section}
        QUESTION: {question}

        Return ONLY a JSON array of exactly 3 short strings.
        """
        raw = self._call_agent(REWRITE_PROMPT, prompt)
        txt = re.sub(r'^```(?:json)?\s*|\s*```$', '', raw.strip(), flags=re.MULTILINE)
        try:
            arr = json.loads(re.search(r'\[.*\]', txt, re.DOTALL).group(0))
            arr = [str(x).strip() for x in arr if str(x).strip()]
        except Exception:
            arr = [l.strip("-*0123456789. ").strip() for l in txt.splitlines() if len(l.strip()) > 20]
        return arr[:3] or [question]
    ...
    def agentic_retrieval(self, question: str, question_id: str = "") -> dict:
        """
        Full agentic retrieval pipeline (case-report collection only):
          1. Plan whether to decompose the question.
          2. Optionally decompose into sub-queries.
          3. Retrieve, then iteratively (gap-aware) reformulate and re-retrieve
        ...
        # Multi-hypothesis HyDE generates the queries; the raw question is kept as a hedge.
        hypotheses = self.hyde_queries(question) if self.enable_query_decomposition else []
        queries = [question] + hypotheses
        if self.verbose:
            print(f"{prefix}[HyDE] {len(hypotheses)} hypotheses: {hypotheses}")
        ...
        def run_retrieval_iteration(queries: List[str], iteration: int) -> bool:
            """Retrieve, optionally critique, log. Returns True if retrieval should stop."""
            nonlocal all_documents

            new_docs = self.retrieve_for_queries(queries)
            all_documents = list(dict.fromkeys(all_documents + new_docs))  # merge + dedupe
        ...
        # Gap-aware reformulation iterations (only if initial pass wasn't sufficient)
        if not sufficient:
            for i in range(1, self.max_iterations):
                ...
                reformulated = self.hyde_queries(question, gaps=last_gaps)
                if self.verbose:
                    print(f"{prefix}[REFORMULATE] gap-targeted hypotheses: {reformulated}")
